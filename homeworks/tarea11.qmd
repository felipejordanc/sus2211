...
Cada carpeta contiene **cinco bandas** del satélite Sentinel-2 (Nivel L1C, *Raw*) que necesitaremos a lo largo de la tarea. Los archivos están prefijados con `t0`, `t1`, `t2` o `t3` según la fecha (`t` = tiempo) — así, una vez cargados en QGIS no se confunden entre sí:

| Archivo | Fecha | Banda | Uso en esta tarea |
|---|---|---|---|
| `t{N}_B02.tiff` | según `N` | Azul (B2) | Canal Azul del color verdadero |
| `t{N}_B03.tiff` | según `N` | Verde (B3) | Canal Verde del color verdadero |
| `t{N}_B04.tiff` | según `N` | Rojo (B4) | Canal Rojo del color verdadero + denominador del NDVI |
| `t{N}_B08.tiff` | según `N` | Infrarrojo cercano — NIR (B8) | NIR para NDVI y NBR |
| `t{N}_B12.tiff` | según `N` | Infrarrojo de onda corta — SWIR (B12) | SWIR para NBR |

donde `N ∈ {0, 1, 2, 3}` corresponde a la fecha de la imagen: `t0` = 2017-01-19 (línea base), `t1` = 2017-03-20, `t2` = 2018-03-20, `t3` = 2024-03-28.

::: callout-tip
**¿Por qué el prefijo `t0..t3`?** Si las cinco bandas de una fecha se llamaran sólo `B02.tiff`, `B03.tiff`, etc., al cargar las 20 bandas de las cuatro fechas en QGIS aparecerían **cuatro veces el mismo nombre** en el panel de Capas — un caos para distinguirlas. El prefijo temporal hace que cada archivo tenga un nombre único (por ejemplo, `t1_B04.tiff` = banda roja de la imagen "casi dos meses después del incendio").
:::
...
7.  **Preguntas guía para el portafolio.** Reflexionar sobre la severidad del incendio, la recuperación a lo largo del tiempo y las ventajas/limitaciones de la teledetección satelital.
...
flowchart TD
    ...
    B --> F["📁 brutos/<br>🔒 4 carpetas SantaOlga_YYYYMMDD<br>con bandas t{0..3}_B{02..12}<br>(no modificar)"]
...
-   **`brutos/`** 🔒 Cuatro carpetas `SantaOlga_YYYYMMDD/`, cada una con las bandas prefijadas según el tiempo: `t0_B02.tiff`…`t0_B12.tiff` para la línea base, `t1_*` para 2017-03-20, `t2_*` para 2018-03-20 y `t3_*` para 2024-03-28. **Nunca se modifican ni se sobreescriben.**
...
![Al terminar deberías tener cuatro grupos en el panel Capas, ordenados de más antiguo a más reciente.](figures/tarea11/tarea11_004.png){fig-align="center"}

**4.** Ahora carga las cinco bandas de cada fecha en su grupo correspondiente. Puedes arrastrar las cinco bandas de la carpeta `SantaOlga_20170119/` (los archivos `t0_B02.tiff`…`t0_B12.tiff`) sobre el grupo `SantaOlga_20170119` en el panel Capas. Repite con `t1_*` en el grupo `SantaOlga_20170320`, `t2_*` en `SantaOlga_20180320` y `t3_*` en `SantaOlga_20240328`.

::: callout-tip
**Ordena las bandas dentro de cada grupo.** Para los pasos siguientes nos conviene que dentro de cada grupo las bandas estén en orden: `t{N}_B02`, `t{N}_B03`, `t{N}_B04`, `t{N}_B08`, `t{N}_B12`. Arrastra las capas dentro del grupo si vienen en otro orden.
:::
...
![Acceso al algoritmo Combinar desde el menú Ráster.](figures/tarea11/tarea11_005.png){fig-align="center"}

**6.** En la ventana del algoritmo, configura:

-   **Capas de entrada:** abre el selector y marca las **cinco bandas** de una misma fecha — por ejemplo `t1_B02`, `t1_B03`, `t1_B04`, `t1_B08` y `t1_B12` del grupo `SantaOlga_20170320`. Mantenlas en ese orden.
...
![Diálogo Combinar — capas de entrada, casilla "Coloca cada archivo de entrada en una banda separada" y tipo de datos Float32.](figures/tarea11/tarea11_008.png){fig-align="center"}
...
![Clic derecho sobre la capa combinada → Exportar → Guardar como…](figures/tarea11/tarea11_009.png){fig-align="center"}
...
![Diálogo "Guardar capa ráster como…": GeoTIFF, nombre `Multibanda_SantaOlga_YYYYMMDD`, en `datos/procesados/`.](figures/tarea11/tarea11_010.png){fig-align="center"}
...
![Una capa multibanda creada y agregada al proyecto, visible bajo su grupo en el panel Capas. Al terminar tendrás una de estas por cada fecha.](figures/tarea11/tarea11_011.jpg){fig-align="center"}

------------------------------------------------------------------------

## Paso 3 | Identificar las bandas del archivo multibanda

Cuando combinamos las cinco bandas en un solo archivo, QGIS las numera **1, 2, 3, 4, 5** dentro de él — perdiendo el nombre original (`t{N}_B02`, `t{N}_B03`, etc.). Para usarlas correctamente en la **Calculadora ráster**, necesitamos saber qué banda interna corresponde a cuál banda Sentinel.
...
![Propiedades → Información. La sección "Bandas" lista las 5 bandas con sus estadísticas (Mín / Máx) — útil para verificar que la combinación se hizo en el orden correcto.](figures/tarea11/tarea11_012.png){fig-align="center"}
...
![Acceso a la Calculadora ráster desde el menú Ráster.](figures/tarea11/tarea11_013.png){fig-align="center"}
...
![Calculadora ráster con la fórmula del NBR aplicada sobre uno de los multibanda (la lista de capas a la izquierda muestra los cuatro multibandas y sus cinco bandas internas).](figures/tarea11/tarea11_014.png){fig-align="center"}
...
![Diálogo Simbología → Color de multibanda con las bandas R/G/B asignadas a las bandas Sentinel B4/B3/B2.](figures/tarea11/tarea11_017.png){fig-align="center"}
...
![Layout 2×2 de los cuatro mapas True Color en orden temporal — la cicatriz del incendio se ve claramente en el segundo cuadrante (2017-03-20).](figures/tarea11/tarea11_020.jpg){fig-align="center"}
...
![Diálogo Simbología → Pseudocolor monobanda para el NDVI: Mín = -0,35, Máx = 0,85, rampa rojo → verde.](figures/tarea11/tarea11_023.png){fig-align="center"}
...
![Diálogo Simbología → Pseudocolor monobanda para el dNBR: Mín = -1,8, Máx = 1,25, rampa que enfatiza el rojo en valores altos.](figures/tarea11/tarea11_029.png){fig-align="center"}

![Resultado del dNBR a 2 meses con la simbología aplicada: los rojos intensos marcan las zonas de mayor severidad del incendio.](figures/tarea11/tarea11_030.jpg){fig-align="center"}
...
Has finalizado los tres mapas. **Recuerda completar esta actividad y guardar tu carpeta para utilizarla en tu próximo portafolio.**

::: callout-important
**Elige el caso de Santa Olga para tu portafolio** y construye una breve narrativa a partir de los tres mapas que produjiste — True Color, NDVI y dNBR en orden temporal. Considera las siguientes preguntas guía:
