...
El objetivo de esta práctica es que **explores la relación entre la vegetación y el calor** en el Gran Santiago combinando dos fuentes de teledetección distintas: **Landsat 8** (NDVI y temperatura superficial, 30 m, NASA/USGS) y el **proyecto Santiago HOT** (temperatura ambiental modelada a 10 m, derivada de ciencia ciudadana + datos satelitales). Trabajarás con **datos ráster y vectoriales** para producir mapas comparativos del Gran Santiago y de una zona específica que tú elijas, e integrarás los resultados en un **Mapa-Memo** que reflexione sobre la distribución espacial del riesgo asociado al calor extremo.
...
8.  **Mapa-Memo.** Integrar los resultados en una reflexión escrita con tres preguntas guía + una pregunta personal vinculada con la actividad en clase.
...
**`documentos/`** — Documentación de respaldo. Recomendado: un `Mapa-Memo.docx` o `bitacora.md` donde registres tus hallazgos y respondas las preguntas guía.
...
Has completado el análisis de temperatura y vegetación. Ahora viene la parte más importante: **interpretar lo que viste y comunicarlo**.

::: callout-important
**Mapa-Memo — entrega final**

En **Canvas**, sube un documento (PDF o Word) con:

1.  **Los tres mapas finales** (Gran Santiago: NDVI vs T° superficial; T° aire vs T° superficial; y los tres pares del zoom a tu zona elegida).
2.  **Las respuestas a las cuatro preguntas guía** (texto breve, 1–2 párrafos por pregunta):

**Pregunta 1.** ¿Qué patrones espaciales de temperatura pudiste observar en el Gran Santiago y en tu zona elegida? ¿Qué factores urbanos (arbolado, densidad construida, pavimento, distancia a la cordillera, etc.) crees que influyen en estas diferencias?

**Pregunta 2.** ¿Cómo se relaciona la distribución de áreas verdes (NDVI) con la temperatura superficial? ¿Qué zonas se benefician más de la vegetación y cuáles se ven más afectadas por su ausencia?

**Pregunta 3.** ¿Qué ventajas y limitaciones tiene el uso de imágenes satelitales como **Landsat 8** para este tipo de análisis urbano? Y, comparativamente, ¿qué aporta un producto como **Santiago HOT** (que combina ciencia ciudadana + satélites) frente a usar sólo datos satelitales?

**Pregunta 4 — vinculación con la clase.** En la actividad **A+R "Plan Calor para tu comuna"** discutimos cuatro comunas con perfiles muy distintos: **Vitacura, Cerro Navia, Santiago Centro y La Pintana**. Si tu grupo trabajó una de ellas (o si tu zona elegida en el Paso 10 corresponde a alguna), ¿cómo se ve esa comuna en los mapas que construiste? ¿Las **tres medidas** que tu grupo eligió tienen sentido a la luz de los datos territoriales que ahora ves? ¿Cambiarías alguna ahora que tienes el mapa en mano?
:::
