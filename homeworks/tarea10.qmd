...
flowchart TD
    ...
    B --> G["📁 intermedios/<br>pasos intermedios del flujo"]
...
-   **`intermedios/`** Archivos derivados que existen sólo como paso intermedio hacia otra capa (nada los consume directamente en el mapa o análisis final). En esta tarea no usaremos `intermedios/`: el clip del Landsat RGB que generarás en el paso 4 se carga tal cual como capa final de referencia, así que va directo a `procesados/`.
-   **`procesados/`** La última versión de cada archivo en su cadena de procesamiento — la capa que efectivamente cargas para construir los mapas. En esta tarea vivirá aquí el **clip** del Landsat RGB (`Landsat8_GS_RGB_clip.tif`) que generarás en el paso 4.
...
-   **Cortado (máscara):** guárdalo en `datos/procesados/` con el nombre `Landsat8_GS_RGB_clip.tif`.

![Diálogo "Cortar ráster por capa de máscara" con las casillas correctas.](figures/tarea10/tarea10_010.png){fig-align="center"}

::: callout-tip
**¿Por qué `procesados/` y no `intermedios/`?** El clip recortado **es** la capa RGB que cargarás para los mapas finales — nada más se construye encima de él. La regla sencilla: si el archivo es la **última versión** que se consume aguas abajo en el flujo, va a `procesados/`. `intermedios/` queda reservado para archivos que existen sólo como peldaño hacia otro derivado posterior.
:::
...
::: callout-note
**Pausa para observar.** Antes de seguir, mira el mapa con detención. Recuerda que en la rampa Magma el **amarillo claro corresponde a las zonas más frescas** y el **rojo oscuro a las más calientes**. ¿Qué comunas se ven más oscuras (más calientes)? ¿Cuáles más amarillas (más frescas)? ¿Hay alguna relación con lo que conoces de la geografía de Santiago (cordillera, valle, sur, norte, oriente, poniente)?
:::
...
::: callout-tip
**Comparación mental.** ¿Las áreas más verdes en el mapa de NDVI coinciden con las áreas más amarillas (frescas) del mapa de temperatura superficial? Anota tu primera impresión — la verificaremos formalmente en el siguiente paso.
:::
...
Para comparar formalmente ambos mapas, vamos a crear una **composición lado a lado** (side-by-side) en el **Compositor de impresión** de QGIS — la misma técnica de díptico horizontal que vimos en detalle en la **Clase 5** y practicamos en el Paso 10 de la **Tarea 5**. Si necesitas refrescar los detalles (orientación apaisada, bloqueo de capas, leyenda doble), vuelve a esa tarea.
...
::: callout-tip
**Tu zona, tu elección.** Por defecto, esta guía usa el **Parque O'Higgins** (comuna de Santiago) como ejemplo, porque es un área verde grande rodeada de zonas densamente construidas — un caso clásico para ver el efecto del arbolado. Pero **tú puedes elegir otra zona** que te interese más: tu comuna, tu barrio, un parque cerca de tu casa, o una de las tres comunas que trabajamos en la actividad de la clase (Vitacura, Cerro Navia, Santiago Centro). La consigna es la misma: **acércate a escala 1:20.000** y produce los tres pares de mapas siguientes.
:::
