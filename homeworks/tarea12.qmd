...
| Títulos de Merced | Vectorial (polígonos) | Descarga (CONADI / SIIC) | Límites históricos de las reducciones mapuche |
...
| `Paleta de colores.clr` | Mapa de color | Insumos | Colores estándar de MapBiomas para el año 2000 |
...
**1.** Descarga el archivo `.zip` de esta tarea desde la página del curso en **Canvas** (carpeta "Clase 12"). Contiene los insumos ya organizados en la estructura estándar: la máscara `REGION_C17`, el archivo `maceda.csv`, la `Paleta de colores.clr` y las imágenes de referencia del año 2000.
...
flowchart TD
    ...
    B --> F["📁 brutos/<br>🔒 REGION_C17, maceda.csv,<br>Paleta de colores.clr, JPG 2000<br>+ descargas: MapBiomas, Títulos de Merced"]
...
-   **`brutos/`** 🔒 Los datos **tal como los obtuviste**, sin modificar: la máscara `REGION_C17`, `maceda.csv`, `Paleta de colores.clr`, las imágenes JPG del año 2000, y los archivos que **descargarás** (el ráster de MapBiomas 2022 y los Títulos de Merced del CONADI). **Nunca se modifican ni se sobreescriben.**
...
## Paso 4 | Descargar y recortar los Títulos de Merced

**11.** Descarga la capa de **Títulos de Merced** desde el Sistema Integrado de Información del CONADI (SIIC 2020): ingresa a [siic.conadi.gob.cl](https://siic.conadi.gob.cl/), dirígete a `Biblioteca Digital → Cubiertas Descargables` y descarga la capa de los Títulos de Merced. Guárdala en `datos/brutos/`.
